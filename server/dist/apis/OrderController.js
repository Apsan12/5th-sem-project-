...
const OrderController = {
    ...
    async updateOrderStatus(req, res) {
        try {
            ...
            const order = await Order.findById(new mongoose.Types.ObjectId(req.body.orderId));
            if (!order)
                return res
                    .status(404)
            ...
            if (req.body.status === "cancelled" ||
                ...
                try {
                    ...
                    if (req.lookup.status !== "Refunded" &&
                        ...
                        !req.lookup.refunded) {
                        await axios.post(`https://dev.khalti.com/api/merchant-transaction/${req.lookup.transaction_id}/refund/`, {
                            mobile: "9800000005",
                        }, {
                            headers: {
    ...
    async cancelOrder(req, res) {
        try {
            ...
            if (user.disabled)
                throw new Error("User is currently disabled");
            const order = await Order.findById(new mongoose.Types.ObjectId(req.body.orderId)).populate("products.product");
            if (!order)
                return res
                    .status(404)
            ...
            try {
                ...
                if (req.lookup.status !== "Refunded") {
                    await axios.post(`https://dev.khalti.com/api/v2/merchant-transaction/${req.lookup.transaction_id}/refund/`, {
                        mobile: "9800000005",
                    }, {
                        headers: {
                            Authorization: `Key ${process.env.KHALTI_AUTH_KEY}`,
